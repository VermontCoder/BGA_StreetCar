-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- Host: mysql
-- Generation Time: Sep 19, 2024 at 10:31 PM
-- Server version: 5.7.44-log
-- PHP Version: 8.2.22

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `ebd_streetcar_623696`
--

-- --
--
-- Truncate table before insert `bga_globals`
--

TRUNCATE TABLE `bga_globals`;
--
-- Dumping data for table `bga_globals`
--

INSERT INTO `bga_globals` (`name`, `value`) VALUES
('curDie', 'null'),
('curTrainDestinationsSelection', 'null'),
('curTrainFacingsTileSelection', 'null'),
('stackIndex', '0');

-- --------------------------------------------------------

--
-- Table structure for table `bga_user_preferences`
--

--
-- Truncate table before insert `bga_user_preferences`
--

TRUNCATE TABLE `bga_user_preferences`;
--
-- Dumping data for table `bga_user_preferences`
--

INSERT INTO `bga_user_preferences` (`pgp_player`, `pgp_preference_id`, `pgp_value`) VALUES
(2383264, 200, 0),
(2383265, 200, 0);

-- --------------------------------------------------------

--
-- Table structure for table `board`
--


--
-- Truncate table before insert `board`
--

TRUNCATE TABLE `board`;
--
-- Dumping data for table `board`
--

INSERT INTO `board` (`board_x`, `board_y`, `directions_free`, `stop`, `card`, `rotation`) VALUES
(0, 0, 'X', NULL, 0, 0),
(0, 1, 'X', NULL, 0, 0),
(0, 2, 'ES', NULL, 1, 270),
(0, 3, 'NE', NULL, 1, 180),
(0, 4, 'X', NULL, 0, 0),
(0, 5, 'X', NULL, 0, 0),
(0, 6, 'ES', NULL, 1, 270),
(0, 7, 'NE', NULL, 1, 180),
(0, 8, 'X', NULL, 0, 0),
(0, 9, 'X', NULL, 0, 0),
(0, 10, 'ES', NULL, 1, 270),
(0, 11, 'NE', NULL, 1, 180),
(0, 12, 'X', NULL, 0, 0),
(0, 13, 'X', NULL, 0, 0),
(1, 0, 'X', NULL, 0, 0),
(1, 1, '[]', NULL, 0, 0),
(1, 2, 'EW', NULL, 0, 0),
(1, 3, 'SW', NULL, 1, 0),
(1, 4, 'NE', 'G', 1, 180),
(1, 5, '[]', NULL, 0, 0),
(1, 6, '[]', NULL, 0, 0),
(1, 7, '[]', NULL, 0, 0),
(1, 8, '[]', NULL, 0, 0),
(1, 9, '[]', NULL, 0, 0),
(1, 10, 'SWE', NULL, 6, 180),
(1, 11, 'NEW', NULL, 4, 0),
(1, 12, '[]', NULL, 0, 0),
(1, 13, 'X', NULL, 0, 0),
(2, 0, 'ES', NULL, 1, 270),
(2, 1, '[]', NULL, 0, 0),
(2, 2, 'ESW', NULL, 3, 90),
(2, 3, 'NES', NULL, 3, 0),
(2, 4, 'NSW', NULL, 2, 0),
(2, 5, 'NE', NULL, 1, 180),
(2, 6, '[]', NULL, 0, 0),
(2, 7, '[]', NULL, 0, 0),
(2, 8, '[]', NULL, 0, 0),
(2, 9, '[]', NULL, 0, 0),
(2, 10, 'SWE', 'F', 4, 180),
(2, 11, 'NEW', NULL, 6, 0),
(2, 12, '[]', NULL, 0, 0),
(2, 13, 'NE', NULL, 1, 180),
(3, 0, 'SW', NULL, 1, 0),
(3, 1, 'ESN', NULL, 6, 90),
(3, 2, 'WN', NULL, 1, 90),
(3, 3, 'EW', NULL, 0, 0),
(3, 4, '[]', NULL, 0, 0),
(3, 5, 'SW', NULL, 1, 0),
(3, 6, 'NESW', NULL, 8, 0),
(3, 7, 'SN', NULL, 0, 90),
(3, 8, '[]', NULL, 0, 0),
(3, 9, '[]', NULL, 0, 0),
(3, 10, '[]', NULL, 0, 0),
(3, 11, 'EW', NULL, 0, 0),
(3, 12, '[]', NULL, 0, 0),
(3, 13, 'NW', NULL, 1, 90),
(4, 0, 'X', NULL, 0, 0),
(4, 1, 'EW', 'H', 0, 0),
(4, 2, '[]', NULL, 0, 0),
(4, 3, 'EW', NULL, 0, 0),
(4, 4, 'ES', NULL, 1, 270),
(4, 5, 'SN', NULL, 0, 90),
(4, 6, 'WN', NULL, 1, 90),
(4, 7, '[]', NULL, 0, 0),
(4, 8, '[]', NULL, 0, 0),
(4, 9, '[]', NULL, 0, 0),
(4, 10, 'ES', NULL, 1, 270),
(4, 11, 'WNES', NULL, 10, 270),
(4, 12, '[]', NULL, 0, 0),
(4, 13, 'X', NULL, 0, 0),
(5, 0, 'X', NULL, 0, 0),
(5, 1, 'EW', NULL, 0, 0),
(5, 2, '[]', NULL, 0, 0),
(5, 3, 'NESW', NULL, 7, 0),
(5, 4, 'SWN', NULL, 3, 180),
(5, 5, 'NES', NULL, 3, 0),
(5, 6, 'SN', NULL, 0, 90),
(5, 7, 'SN', 'M', 0, 90),
(5, 8, 'NE', NULL, 1, 180),
(5, 9, '[]', NULL, 0, 0),
(5, 10, 'WNES', NULL, 8, 270),
(5, 11, 'EWN', 'E', 2, 90),
(5, 12, '[]', NULL, 0, 0),
(5, 13, 'X', NULL, 0, 0),
(6, 0, 'ES', NULL, 1, 270),
(6, 1, 'SWNE', NULL, 7, 180),
(6, 2, 'SN', NULL, 0, 90),
(6, 3, 'EWN', 'I', 2, 90),
(6, 4, '[]', NULL, 0, 0),
(6, 5, 'ESW', NULL, 3, 90),
(6, 6, 'ESN', NULL, 4, 90),
(6, 7, '[]', NULL, 0, 0),
(6, 8, 'SWE', NULL, 4, 180),
(6, 9, 'SN', NULL, 0, 90),
(6, 10, 'SWNE', NULL, 9, 180),
(6, 11, 'NESW', NULL, 7, 0),
(6, 12, 'SN', NULL, 0, 90),
(6, 13, 'NE', NULL, 1, 180),
(7, 0, 'SW', NULL, 1, 0),
(7, 1, 'WN', 'A', 1, 90),
(7, 2, '[]', NULL, 0, 0),
(7, 3, 'NEW', NULL, 6, 0),
(7, 4, '[]', NULL, 0, 0),
(7, 5, 'NEW', NULL, 4, 0),
(7, 6, 'EW', NULL, 0, 0),
(7, 7, '[]', NULL, 0, 0),
(7, 8, '[]', NULL, 0, 0),
(7, 9, '[]', NULL, 0, 0),
(7, 10, 'SWE', 'L', 6, 180),
(7, 11, 'ESWN', NULL, 5, 90),
(7, 12, 'ESN', NULL, 4, 90),
(7, 13, 'NW', NULL, 1, 90),
(8, 0, 'X', NULL, 0, 0),
(8, 1, '[]', NULL, 0, 0),
(8, 2, 'ES', NULL, 1, 270),
(8, 3, 'NESW', NULL, 7, 0),
(8, 4, 'SN', NULL, 0, 90),
(8, 5, 'SWNE', NULL, 8, 180),
(8, 6, 'WN', 'K', 1, 90),
(8, 7, '[]', NULL, 0, 0),
(8, 8, '[]', NULL, 0, 0),
(8, 9, '[]', NULL, 0, 0),
(8, 10, 'EW', NULL, 0, 0),
(8, 11, '[]', NULL, 0, 0),
(8, 12, 'NEW', NULL, 6, 0),
(8, 13, 'X', NULL, 0, 0),
(9, 0, 'X', NULL, 0, 0),
(9, 1, 'ES', NULL, 1, 270),
(9, 2, 'NESW', NULL, 9, 0),
(9, 3, 'NEW', NULL, 6, 0),
(9, 4, '[]', NULL, 0, 0),
(9, 5, 'EW', NULL, 0, 0),
(9, 6, '[]', NULL, 0, 0),
(9, 7, '[]', NULL, 0, 0),
(9, 8, '[]', NULL, 0, 0),
(9, 9, '[]', NULL, 0, 0),
(9, 10, 'WE', NULL, 0, 180),
(9, 11, '[]', NULL, 0, 0),
(9, 12, 'EW', NULL, 0, 0),
(9, 13, 'X', NULL, 0, 0),
(10, 0, 'ES', NULL, 1, 270),
(10, 1, 'WN', NULL, 1, 90),
(10, 2, 'EW', NULL, 0, 0),
(10, 3, 'SWE', NULL, 6, 180),
(10, 4, 'SN', NULL, 0, 90),
(10, 5, 'NESW', NULL, 10, 0),
(10, 6, 'SN', NULL, 0, 90),
(10, 7, 'NE', NULL, 1, 180),
(10, 8, 'ES', NULL, 1, 270),
(10, 9, 'NS', NULL, 0, 270),
(10, 10, 'WNES', NULL, 8, 270),
(10, 11, 'SN', 'D', 0, 90),
(10, 12, 'SWN', NULL, 3, 180),
(10, 13, 'NE', NULL, 1, 180),
(11, 0, 'SW', NULL, 1, 0),
(11, 1, 'NE', NULL, 1, 180),
(11, 2, '[]', NULL, 0, 0),
(11, 3, 'WE', NULL, 0, 180),
(11, 4, '[]', NULL, 0, 0),
(11, 5, 'EW', NULL, 0, 0),
(11, 6, '[]', NULL, 0, 0),
(11, 7, 'WES', NULL, 2, 270),
(11, 8, 'WN', NULL, 1, 90),
(11, 9, '[]', NULL, 0, 0),
(11, 10, 'EW', NULL, 0, 0),
(11, 11, '[]', NULL, 0, 0),
(11, 12, 'ES', NULL, 1, 270),
(11, 13, 'NW', NULL, 1, 90),
(12, 0, 'X', NULL, 0, 0),
(12, 1, 'SW', NULL, 1, 0),
(12, 2, 'NE', NULL, 1, 180),
(12, 3, 'WES', NULL, 2, 270),
(12, 4, 'NS', 'B', 0, 270),
(12, 5, 'NSW', NULL, 2, 0),
(12, 6, 'NE', NULL, 1, 180),
(12, 7, 'EW', 'C', 0, 0),
(12, 8, '[]', NULL, 0, 0),
(12, 9, '[]', NULL, 0, 0),
(12, 10, 'NESW', NULL, 7, 0),
(12, 11, 'ESN', NULL, 4, 90),
(12, 12, 'WN', NULL, 1, 90),
(12, 13, 'X', NULL, 0, 0),
(13, 0, 'X', NULL, 0, 0),
(13, 1, 'X', NULL, 0, 0),
(13, 2, 'SW', NULL, 1, 0),
(13, 3, 'NW', NULL, 1, 90),
(13, 4, 'X', NULL, 0, 0),
(13, 5, 'X', NULL, 0, 0),
(13, 6, 'SW', NULL, 1, 0),
(13, 7, 'NW', NULL, 1, 90),
(13, 8, 'X', NULL, 0, 0),
(13, 9, 'X', NULL, 0, 0),
(13, 10, 'SW', NULL, 1, 0),
(13, 11, 'NW', NULL, 1, 90),
(13, 12, 'X', NULL, 0, 0),
(13, 13, 'X', NULL, 0, 0);

-- --------------------------------------------------------

--
-- Table structure for table `gamelog`
--


--
-- Truncate table before insert `gamelog`
--

TRUNCATE TABLE `gamelog`;
--
-- Dumping data for table `gamelog`
--

-- --------------------------------------------------------

--
-- Table structure for table `global`
--


--
-- Truncate table before insert `global`
--

TRUNCATE TABLE `global`;
--
-- Dumping data for table `global`
--

INSERT INTO `global` (`global_id`, `global_value`) VALUES
(0, 1726785095),
(1, 12),
(2, 2383264),
(3, 83),
(4, 10202),
(5, 2383264),
(6, 13),
(7, 0),
(8, 180),
(9, 40),
(200, 1),
(201, 0),
(300, 999999999),
(301, 0),
(302, 0),
(304, 0),
(305, 0),
(306, 0);

-- --------------------------------------------------------

--
-- Table structure for table `player`
--


--
-- Truncate table before insert `player`
--

TRUNCATE TABLE `player`;
--
-- Dumping data for table `player`
--

INSERT INTO `player` (`player_no`, `player_id`, `player_canal`, `player_name`, `player_avatar`, `player_color`, `player_score`, `player_score_aux`, `player_zombie`, `player_ai`, `player_eliminated`, `player_next_notif_no`, `player_enter_game`, `player_over_time`, `player_is_multiactive`, `player_start_reflexion_time`, `player_remaining_reflexion_time`, `player_beginner`, `player_state`, `available_cards`, `linenum`, `goals`, `goalsfinished`, `trainposition`, `traindirection`, `endnodeids`, `laststopnodeid`, `dice`, `diceused`) VALUES
(1, 2383264, '9215eef7d222e79a42d1f81e6c87b483', 'FindYodaWinCash0', '000000', 'ff0000', 0, 0, 0, 0, 0, 1, 1, 1, 0, '2024-09-20 00:31:35', -1120098, 0xffffffffffffffffffffffffffffffff, NULL, '[0,0,0,1,1]', 2, '[]', '[\"L\",\"M\",\"B\"]', '4_1_W', 'E', '[\"6_0_S\",\"7_0_S\"]', '4_1_W', '[2]', 2),
(2, 2383265, '6d5912ba6209bd8a002f21353eb338db', 'FindYodaWinCash1', '000000', '008000', 0, 0, 0, 0, 0, 1, 1, 1, 0, NULL, -1104, 0xffffffffffffffffffffffffffffffff, NULL, '[0,0,0,1,1]', 5, '[\"D\",\"I\"]', '[\"F\"]', '2_10_W', 'S', '[\"13_6_W\",\"13_7_W\"]', '2_10_W', NULL, 0);

-- --------------------------------------------------------

--
-- Table structure for table `replaysavepoint`
--

--
-- Truncate table before insert `replaysavepoint`
--

TRUNCATE TABLE `replaysavepoint`;
--
-- Dumping data for table `replaysavepoint`
--


--
-- Table structure for table `stack`
--

--
-- Truncate table before insert `stack`
--

TRUNCATE TABLE `stack`;
--
-- Dumping data for table `stack`
--

INSERT INTO `stack` (`id`, `card`) VALUES
(0, 0),
(1, 1),
(2, 1),
(3, 4),
(4, 0),
(5, 1),
(6, 9),
(7, 0),
(8, 4),
(9, 0),
(10, 9),
(11, 4),
(12, 0),
(13, 4),
(14, 0),
(15, 2),
(16, 0),
(17, 8),
(18, 0),
(19, 3),
(20, 11),
(21, 7),
(22, 8),
(23, 8),
(24, 4),
(25, 3),
(26, 1),
(27, 3),
(28, 5),
(29, 1),
(30, 3),
(31, 1),
(32, 7),
(33, 3),
(34, 0),
(35, 0),
(36, 2),
(37, 1),
(38, 1),
(39, 0),
(40, 5),
(41, 4),
(42, 1),
(43, 4),
(44, 7),
(45, 1),
(46, 6),
(47, 2),
(48, 0),
(49, 0),
(50, 1),
(51, 0),
(52, 5),
(53, 4),
(54, 1),
(55, 7),
(56, 7),
(57, 2),
(58, 1),
(59, 6),
(60, 5),
(61, 3),
(62, 2),
(63, 1),
(64, 0),
(65, 2),
(66, 0),
(67, 0),
(68, 10),
(69, 3),
(70, 10),
(71, 1),
(72, 1),
(73, 0),
(74, 6),
(75, 0),
(76, 11),
(77, 6),
(78, 5),
(79, 9),
(80, 2),
(81, 5),
(82, 8),
(83, 3),
(84, 0),
(85, 2),
(86, 3),
(87, 1),
(88, 7),
(89, 3),
(90, 1),
(91, 0),
(92, 1),
(93, 1),
(94, 4),
(95, 2),
(96, 2),
(97, 6),
(98, 9),
(99, 4);

-- --------------------------------------------------------

--
-- Table structure for table `stats`
--

--
-- Truncate table before insert `stats`
--

TRUNCATE TABLE `stats`;
--
-- Dumping data for table `stats`
--

INSERT INTO `stats` (`stats_id`, `stats_type`, `stats_player_id`, `stats_value`) VALUES
(1, 1, 2383264, 1120280),
(2, 1, 2383265, 1284);

--

COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
