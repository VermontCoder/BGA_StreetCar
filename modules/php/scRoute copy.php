...
{    
    ...
    */
    ...
    public function merge($otherRoute)
    {
        ...
        $mergedRoute->routeNodes[$this->endNodeID.'_'.$this->routeID] = $otherRoute->startNodeID.'_'.$otherRoute->routeID; //$otherRoute->routeNodes[$otherRoute->startNodeID.'_'.$otherRoute->routeID];

        //remove the start node of the other route.
        //unset($mergedRoute->routeNodes[$otherRoute->startNodeID.'_'.$otherRoute->routeID]);

        // foreach ($this->routeNodes as $parentNode => $childNode)
        // {
        //     //construct new routeNodeIDs so the first route routeids are the same as the merged route id.
        //     //DO NOT REMOVE, there is a bug if you use the same routeID for the merged route over and over.

        //     //This whole problem could be avoided by just using the merged routeID, but then we couldn't find the startnode.
        //     //THIS IS VERY UGLY. SORRY.
        //     $p = explode('_',$parentNode);
        //     $c = explode('_', $childNode);

        //     if ($p[3] == strval($this->routeID))
        //     {
        //         $newParentRouteID = $p[0].'_'.$p[1].'_'.$p[2].'_'.strval($mergedRoute->routeID);
        //         $newChildRouteID = $c[0].'_'.$c[1].'_'.$c[2].'_'.strval($mergedRoute->routeID);
        //         unset($mergedRoute->routeNodes[$parentNode]);
                
        //         $mergedRoute->routeNodes[$newParentRouteID] = $newChildRouteID;
